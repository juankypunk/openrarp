...
$query="SELECT id_parcela,titular_cc,cc,to_char(fecha,'DD-MM-YY') as fecha,
	...
	FROM vista_agua where fecha=(SELECT MAX(fecha) FROM agua) AND importe > 0 AND domiciliado > 0";
//echo $query;
...
while ($i < $num_filas) {
	...
	$AT10='AGUA'.$fecha_cobro.$id_parcela; //referencia del adeudo
	$C1='03';
	$C2='19143';
	$C3='003';
	...
	$C7=$AT59;
	$total+=$fila['importe'];
	$importe_f=str_replace('.','',$fila['importe']);
	$importe_f=trim($importe_f);
	$C8=str_pad($importe_f,11,"0",STR_PAD_LEFT);
...
echo 'Fichero: <a href="remesas/remesa_agua_sepa.dat" title="remesa_agua_sepa.dat">Descargar fichero</a><br>';
$contenido.= $R1.$R2.$R3.$R4.$R5.$R6;

if (is_writable('../remesas/remesa_agua_sepa.dat')) {
	if (!$gestor = fopen('../remesas/remesa_agua_sepa.dat', 'w')) {
		echo "No se puede abrir el archivo remesa_agua_sepa.dat";
		exit;
	}else{
